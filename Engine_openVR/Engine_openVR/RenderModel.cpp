...
RenderModel::RenderModel(const char* rm_name) {
	...
	this->name = new char[std::strlen(rm_name)]; 
	std::strcpy(this->name, rm_name);
...
bool RenderModel::init() {
	...
	while (1)
	{
		std::cout << "Starting loading render model's model (" << name << ")" << std::endl;
	...
	{
		std::cout << "Unable to load render model " << name << ". Error code: " << vr::VRRenderModels()->GetRenderModelErrorNameFromEnum(error);
	...
	while (1)
	{
		std::cout << "Starting loading render model's texture (" << name << ")" << std::endl;
	...
	if (error != vr::VRRenderModelError_None)
	{
		std::cout << "Unable to load render texture id " << model->diffuseTextureId << " for render model " << name;
		vr::VRRenderModels()->FreeRenderModel(model);
		return false;
	}
...
bool RenderModel::setup_buffers(const vr::RenderModel_t &rm_model, const vr::RenderModel_TextureMap_t &rm_texture)
{
	vertex_count = rm_model.unTriangleCount * 3;

	std::cout << "Starting filling data buffers for render model..." << std::endl;
	...
	glVertexAttribPointer(0, 4, GL_FLOAT, GL_FALSE, sizeof(vr::RenderModel_Vertex_t), (void *)offsetof(vr::RenderModel_Vertex_t, vPosition));
	glEnableVertexAttribArray(1);
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(vr::RenderModel_Vertex_t), (void *)offsetof(vr::RenderModel_Vertex_t, vNormal));
	glEnableVertexAttribArray(2);
	glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(vr::RenderModel_Vertex_t), (void *)offsetof(vr::RenderModel_Vertex_t, rfTextureCoord));

	std::cout << "Vertex buffer filled succesfully" << std:: endl;
	...
	std::cout << "Index buffer filled succesfully" << std::endl;
	...
	std::cout << "Texture successfully registered" << std::endl;
